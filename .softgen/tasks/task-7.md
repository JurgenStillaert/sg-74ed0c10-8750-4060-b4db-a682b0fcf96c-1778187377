---
title: 'Onder "het spel": nieuwe tabs'
status: done
priority: urgent
type: feature
created_by: human
created_at: '2026-05-08T06:25:48.897245'
position: 6
---

## Notes
Maak twee nieuwe tabs aan met de volgende content:
- Verdiensten: verhuis daar de volgende secties naartoe:
   - Waar "De reis begint" in staat
   - Streak
   - Achievements (hernoemen naar prestaties)

- Analyse
   - Totaal Deficit Voortgang
   - Voortgang Grafiek
   - Geschatte Einddatum
   - Cumulatief Calorie Deficit
   - Lichaamssamenstelling

Toon de secties over de dagen (groene, rode en jokers) enkel onder "Kalender overzicht". Dus verwijderen uit Het spel

## Checklist
- [x] GameTabs component gemaakt met Verdiensten en Analyse tabs
- [x] Achievements verplaatst naar Verdiensten tab en hernoemd naar Prestaties
- [x] ProgressCharts verplaatst naar Analyse tab
- [x] YearStats verwijderen uit Het spel tab (blijft alleen in Kalender)
- [x] Alle analyse secties correct tonen in Analyse tab
